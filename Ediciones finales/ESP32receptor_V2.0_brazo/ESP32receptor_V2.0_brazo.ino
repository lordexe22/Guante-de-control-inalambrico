...
// Puertos de entrada de los sensores
#define S1 22
#define S2 1
#define S3 3
#define S4 21
...
// Puertos de salida de los motores
#define M_GX 13
#define M_GY 12
#define M_GZ 14
#define M_D1 27 // Pulgar
#define M_D2 26 // Indice
#define M_D3 25 // Medio
#define M_D4 33 // Menique
...
{  
  ...
  { // Codigo a ejecutar por defecto - Lee los sensores conectados al receptor
    if(digitalRead(S1)==HIGH) reproducirRutina(getRoutine(1));
    /*else if(digitalRead(S2)==HIGH) reproducirRutina(getRoutine(2));
    else if(digitalRead(S3)==HIGH) reproducirRutina(getRoutine(3));
    else if(digitalRead(S4)==HIGH) reproducirRutina(getRoutine(4));
    else if(digitalRead(S5)==HIGH) reproducirRutina(getRoutine(5));
    else if(digitalRead(S6)==HIGH) reproducirRutina(getRoutine(6));
    else if(digitalRead(S7)==HIGH) reproducirRutina(getRoutine(7));
    else if(digitalRead(S8)==HIGH) reproducirRutina(getRoutine(8));
    else if(digitalRead(S9)==HIGH) reproducirRutina(getRoutine(9));        */
  }
} 
...
{ // Reproduce (si existe) la rutina numero "num" de la memoria SD 
  ...
  { // Ejecutamos la rutina indicada por el sensor solo si esta existe
    ...
    while(archivo.available() && !b_forzarCierre)
    { // Leemos el archivo mientras tengamos contenido en el mismo y no se fuerce el cierre de la rutina
      ...
      { // Si el caracter no fue valido, transmitimos los almacenado hasta ahora y limpiamos el String dato
...
int PULSOMIN = 500; // Valores expresados en microsegundos 
int PULSOMAX = 2500;

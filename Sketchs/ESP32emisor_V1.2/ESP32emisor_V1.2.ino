 ...
 *                            (6 = reproducir rutina) (reproduce la rutina seleccionada en el brazo)
 * 
 *    R >> Numero de la rutina seleccionada para los estados (2, 4, 5), en caso de ser innecesaria vale cero
 * 
 *    S >> Numero del sensor seleecionado para los estados (3, 4, 6), en caso de ser innecesaria vale cero
 ****************************************************************************************************************** 
 *
...
#define DELAY_TRANS_MODO_6 3 // Delay entre los sucesivos envios del modo 6 (reproducir rutina) editar y ajustar este valor
...
uint8_t RxMACaddress[] = {0x78 , 0x21 , 0x84 , 0x91 , 0x2D , 0xD0}; 

// ESTRUCTURAS ----------------------------------------------------------------------------------------------------
struct s_sendData
{ // Estructura implementada para la transmision de datos
  short sensor, rutina, estado;
  float ax,ay,az,gx,gy,gz;
  float d1,d2,d3,d4,d5;
  bool b_esDato; // Booleano que indica si la estructura enviada son datos de sensores o no
};
...
bool B_E6_P1 = false; // Booleano auxiliar que indica que debemos transmitir la configuracion de la rutina 6
bool B_E6_P2 = false; // Booleano auxiliar que indica que debemos transmitir el contenido de la rutina seleccionada
bool B_E6_P3 = false; // Booleano auxiliar que indica la finalizacion del estado 6
...
// Gestiona el funcionamiento del programa en el modo 6 - (Reproducir rutina desde la memoria SD - transmisor)
void rutina_G();
...
void loop() 
{ 
  rutina_A(); // Rutina para leer el puerto serial y armar la estructura mySendData
  ...
  rutina_G(); // Gestiona el funcionamiento del programa en el modo 6 - (Reproducir rutina desde la memoria SD - transmisor)
...
{ // Actualiza el archivo lista.txt con el cambio y elimina el archivo de la rutina seleccionado
  ...
  short rutina = mySendData.rutina; // Numero de la rutina que deseamos 
...
{ // Crea una nueva rutina y comienza a almacenar los datos de los sensores >> estado 3 parte 2
  ...
  short rutina = mySendData.rutina; // Numero de la rutina que deseamos crear
...
// ----------------------------------------------------------------------------------------------------------------
void editarArchivoLista()
{ // Edita el contenido del archivo lista.txt segun lo almacenado en la estructura mySendData
  short sensor = mySendData.sensor + 48; // Valor leido del sensor (en ascii)
  short rutina = mySendData.rutina; // Valor leido de la rutina
...
{ // Lee el contenido del puerto serial, asigna la cadena al String str_puertoSerial y retorna un valor entero acorde a la cadena recibida 
  ...
  if(str_puertoSerial[1]=='-')
  {
    ...
    else if(str_puertoSerial[0]=='6') return 6;
    else return 0;
  }
  else return 0;
}
...
{ // Controla los booleanos del programa de manera que trabajen ordenadamente con el programa
  ...
  short estadoActual = mySendData.estado; 
  ...
  // ---------- Gestion del estado 6 ----------
  if(estadoActual!=estadoAnterior && estadoActual==6)
  { // Si acaba de cambiar el estado al modo 6 >> Enviamos la configuracion
    Serial.println("gestorDeBooleanos >> B_E6_P1 = true;");
    B_E6_P1 = true;
    mySendData.b_esDato = false;
    instanteDeCambio = millis(); // Almacenamos el instante donde se dio el cambio de estado    
  } 
  else if(B_E6_P1 && (millis() - instanteDeCambio > DELAY_SENDCONFIG))
  { // Se activa para iniciar la transmision de datos acorde a la rutina seleccionada (transmitimos mySendData)
    Serial.println("gestorDeBooleanos >> B_E6_P1 = false;");
    Serial.println("gestorDeBooleanos >> B_E6_P2 = true;");
    B_E6_P1 = false;
    B_E6_P2 = true;
    mySendData.b_esDato = true;
    instanteDeCambio = millis(); // Almacenamos el instante donde se dio el cambio de estado    
  ...
  else if(B_E6_P2 && estadoActual==0)
  { // Se activa para iniciar la transmision de la configuracion de la estructura mySendData, indicando el fin de la rutina
    Serial.println("gestorDeBooleanos >> B_E6_P2 = false;");
    Serial.println("gestorDeBooleanos >> B_E6_P3 = true;");
    B_E6_P2 = false;
    B_E6_P3 = true;
    mySendData.b_esDato = false;
    instanteDeCambio = millis(); // Almacenamos el instante donde se dio el cambio de estado        
  ...
  else if(B_E6_P3 && estadoActual==0 && (millis() - instanteDeCambio > DELAY_SENDCONFIG))
  { // Para finalizar volvemos a enviar la estructura mySendConfig
    Serial.println("gestorDeBooleanos >> B_E2_P3 = false;");
    B_E6_P3 = false;
    secuencia_A();
...
{ // Lee todos los sensores y los asigna a su respectivo campo dentro de la estructura instanciada mySendData 
  ...
  if(mySendData.estado == 1 || mySendData.estado == 3)
  { //En modo 1 (leer en tiempo real), almacenamos los datos en la estructura mySendData (datos para transmitir)
    ...
    mySendData.ax = (float) a.acceleration.x;    
    mySendData.ay = (float) a.acceleration.y;    
    mySendData.az = (float) a.acceleration.z;    
    mySendData.gx = (float) g.gyro.x;    
    mySendData.gy = (float) g.gyro.y;    
    mySendData.gz = (float) g.gyro.z;    
...
{ // Lectura y manipulacion de datos del puerto serial
  ...
  { // Sentencia de lectura que se ejecuta si el ESP32 resive un mensaje por el puerto serial
    ...
    { // Si el mensaje tiene el formato adecuado, almacenamos los valores a la estructura ya instanciada mySendConfig
      mySendData.estado = getState();
      mySendData.rutina = getRoutine();
      mySendData.sensor = getSensor();
    } 
    else
...
{ // Gestiona el funcionamiento del programa en el modo 1 - (transmision de datos en tiempo real)
  ...
  if(mySendData.estado == 1 || B_E1_P3)
  { // Se activa si estamos en el modo de operacion 1
    ...
    if(B_E1_P1 && (millis() - instanteDeCambio > DELAY_TRANSCONFIG))
    { // Envia a lo largo de (DELAY_SENDCONFIG) segundos la estructura mySendConfig cada (DELAY_TRANSCONFIG) segundos  
      transmitirDatos();
      instanteDeCambio = millis();     
    }    
    else if(B_E1_P2 && (millis() - instanteDeCambio > DELAY_TRANSDATA))
    { // Se ejecuta si ya paso el tiempo en el que se envia la configuracion y transmite mySendData cada (DELAY_TRANSDATA) segundos  
        leerSensores();
        transmitirDatos();
        instanteDeCambio = millis();      
    } 
    else if(B_E1_P3 && (millis() - instanteDeCambio > DELAY_TRANSCONFIG))
    { // Envia a lo largo de (DELAY_SENDCONFIG) segundos la estructura mySendConfig cada (DELAY_TRANSCONFIG) segundos  
      transmitirDatos();
      instanteDeCambio = millis();     
    } 
  } // fin >> if(mySendData.estado == 1)
  else if(B_E1_P4 && finEstado)
  { // Reproducimos la secuencia_A() al finalizar la transmision de datos
    finEstado = false;
    secuencia_A();
  }
  //leerSensores();
  //Serial.println(mySendData.ax);
}
...
{ // Gestiona el funcionamiento del programa en al modo 4 - (cambiar el sensor de alguna rutina)
  ...
  { // Actualiza la lista agregando el nuevo valor del sensor con los valores leidos del puerto serial
    editarArchivoLista(); // Edita el contenido del archivo lista.txt segun lo almacenado en la estructura mySendData
  } // fin >> if(B_E4_P1)
  else if(B_E4_P2)
  { // Transmitimos la estructura mySendData durante el tiempo que dura activo B_E4_P2
    transmitirDatos(); // Transmite los datos almacenados en la instancia de la estructura mySendData
...
{ // Gestiona el funcionamiento del programa en el modo 2 - (Transmitir rutina desde la memoria SD)
  ...
  { // Enviamos cada DELAY_TRANSCONFIG un pack del tipo mySendData
    ...
    mySendData.sensor = lista[2*mySendData.rutina-1]-48; 
    transmitirDatos();
    ultimoEnvio = millis();
  } 
  else if(B_E2_P2)
...
{ // Gestiona el funcionamiento del programa en el modo 6 - (Reproducir rutina desde la memoria SD - transmisor)
  ...
  // LISTA DE TAREAS
  // 1 - Esta funcion solo estara activa si mySendData.estado=6
  // 2 - En el primer loop debe identificar la existencia de la rutina que deseamos transmitir y en caso de que exista
  //     enviamos durante algunos ciclos configuracion pertinente
  // 3 - Despues de enviar la configuracion, procedemos a leer el archivo y almacenar los datos en la estructura mySendData
  //     finalmente enviamos el paquete y repetimos el paso 3 hasta finalizar la lectura del archivo
  // 4 - Para finalizar esta rutina, volvemos e enviar algunas veces el paquete de datos mySendConfig
  ...
  if(B_E6_P1 && millis() - ultimoEnvio > DELAY_TRANSCONFIG) 
  { // Enviamos cada DELAY_TRANSCONFIG un pack del tipo mySendData
    Serial.println("3333333333333333333333333333333333333333333333333333333333333333333333");
    transmitirDatos();
    ultimoEnvio = millis();
  } 
  else if(B_E6_P2)
  { // Cargamos los datos en mySendData leyendo el archivo de la rutina especificado y lo transmitimos
    Serial.println("44444444444444444444444444444444444444444444444444444444444444444444444");
    transmitirRutina();
    ultimoEnvio = millis();
  }
  else if(B_E6_P3 && millis() - ultimoEnvio > DELAY_TRANSCONFIG)
  { // Enviamos cada DELAY_TRANSCONFIG un pack del tipo mySendData
    Serial.println("5555555555555555555555555555555555555555555555555555555555555555555555555");
    transmitirDatos();
    ultimoEnvio = millis();
  } 
}
...
{ // transmite los datos contenidos en el archivo de rutina especificado mediante estructuras mySendData
  ...
  short rutina = mySendData.rutina; // Numero de la rutina que deseamos leer
  ...
  { // leemos hasta el final del archivo  
    ...
    { // Si leemos un dato invalido, guardamos el contenido leido hasta ahora en algun miembro de la estructura mySendData
      if(selector == 1 && cargarDato)       { mySendData.ax = dato.toFloat(); selector = 2;  dato=""; }
      else if(selector == 2 && cargarDato)  { mySendData.ay = dato.toFloat(); selector = 3;  dato=""; }
      else if(selector == 3 && cargarDato)  { mySendData.az = dato.toFloat(); selector = 4;  dato=""; }
      else if(selector == 4 && cargarDato)  { mySendData.gx = dato.toFloat(); selector = 5;  dato=""; }
      else if(selector == 5 && cargarDato)  { mySendData.gy = dato.toFloat(); selector = 6;  dato=""; }
      else if(selector == 6 && cargarDato)  { mySendData.gz = dato.toFloat(); selector = 7;  dato=""; }
      else if(selector == 7 && cargarDato)  { mySendData.d1 = dato.toFloat(); selector = 8;  dato=""; }
      else if(selector == 8 && cargarDato)  { mySendData.d2 = dato.toFloat(); selector = 9;  dato=""; }
      else if(selector == 9 && cargarDato)  { mySendData.d3 = dato.toFloat(); selector = 10; dato=""; }
      else if(selector == 10 && cargarDato) { mySendData.d4 = dato.toFloat(); selector = 11; dato=""; }
      else if(selector == 11 && cargarDato) { mySendData.d5 = dato.toFloat(); selector = 1;  dato=""; transmitirDatos(); }   
      cargarDato=false;     
    }      
    delay(DELAY_TRANS_MODO_6); // Agrego tiempo para poder procesar estos datos en el receptor
  } // fin >> while(archivo.available()) 
  archivo.close();
  mySendData.estado = 0;
}
...
{ // Retorna un booleano que indica si el mensaje cumple con la sintaxis preestablecida, si no cumple con la sintxis o
  ...
  Serial.println("1111111111111111111111111111111111111111111111111111111111111111111111111111111111111");
  Serial.print("estado = "); Serial.println(estado);
  Serial.print("rutina = "); Serial.println(rutina);
  ...
  // Si se quiere reproducir una rutina (estado 6) que no existe
  if(estado==6 && lista[2*rutina-1]==48)  return false;

...
{ // Reproduce (si existe) la rutina numero "num" de la memoria SD 
  ...
  float datosEnMemoriaAnterior[11];
    ...
    { // Leemos el archivo mientras tengamos contenido en el mismo y no se fuerce el cierre de la rutina con BTN_DETENER_RUTINA
      ...
      { // Si el caracter no fue valido, transmitimos lo almacenado hasta ahora y limpiamos el String dato        
        ...
        { // Completamos un paquete de datos, en este condicionar se tienen que procesar los datos y ejecutarse para mover los motores
          for(int j=1; j<=20; j++)
          { // Implementa nuevas medidas para suavizar el movimiento del brazo
            // Movimiento del pulgar
            PWMd1(datosEnMemoriaAnterior[6]+j*(datosEnMemoria[6]-datosEnMemoriaAnterior[6])/20);
            // Movimiento del indice
            PWMd2(datosEnMemoriaAnterior[7]+j*(datosEnMemoria[7]-datosEnMemoriaAnterior[7])/20);
            // Movimiento del medio
            PWMd3(datosEnMemoriaAnterior[8]+j*(datosEnMemoria[8]-datosEnMemoriaAnterior[8])/20);
            // Movimiento del meñique
            PWMd4(datosEnMemoriaAnterior[10]+j*(datosEnMemoria[10]-datosEnMemoriaAnterior[10])/20);
            // Movimiento de elevacion
            PWMelevacion(datosEnMemoriaAnterior[1]+j*(datosEnMemoria[1]-datosEnMemoriaAnterior[1])/20);
            // Movimiento de rotacion
            PWMrotacion(datosEnMemoriaAnterior[4]+j*(datosEnMemoria[4]-datosEnMemoriaAnterior[4])/20);
            // Movimiento de flexion
            PWMflexion(datosEnMemoriaAnterior[3]+j*(datosEnMemoria[3]-datosEnMemoriaAnterior[3])/20);

            delay(20);
          }

          datosEnMemoriaAnterior[0] = datosEnMemoria[0];
          datosEnMemoriaAnterior[1] = datosEnMemoria[1];
          datosEnMemoriaAnterior[2] = datosEnMemoria[2];
          datosEnMemoriaAnterior[3] = datosEnMemoria[3];
          datosEnMemoriaAnterior[4] = datosEnMemoria[4];
          datosEnMemoriaAnterior[5] = datosEnMemoria[5];
          datosEnMemoriaAnterior[6] = datosEnMemoria[6];
          datosEnMemoriaAnterior[7] = datosEnMemoria[7];
          datosEnMemoriaAnterior[8] = datosEnMemoria[8];
          datosEnMemoriaAnterior[9] = datosEnMemoria[9];
          datosEnMemoriaAnterior[10] = datosEnMemoria[10];

...
bool setRutina=true; // Indica que de presionarse un numero, se actualizara el campo rutina
bool setSensor=false; // Indica que de presionarse un numero, se actualizara el campo sensor

short sensor=0; // Contiene el valor actual del campo sensor
short rutina=0; // Contiene el valor actual del campo rutina
...
// Apaga los botones pulsados si paso un tiempo lo suficiente largo y no se volvieron a pulsar
void apagarBotones();

// Agrega efectos al pulsar alguno de los botones de rutinas
void botonPulsado(short boton);
...
{ // Apaga los botones pulsados si paso un tiempo lo suficiente largo y no se volvieron a pulsar
  ...
  if(estadoActual==1 && millis()-ultimaLectura > DELAY_OFF)
  { // Paso 1 segundo desde que se pulso el boton de transmitir en tiempo real(lo desactivamos)
    dibujarBoton(1,0,0);
    dibujarIcono(1,CUADRO,CUADRO);
    estadoActual=0;
  }
  else if(estadoActual==2 && millis()-ultimaLectura > DELAY_OFF)
  { // Paso 1 segundo desde que se pulso el boton de crear una nueva rutina (lo desactivamos)
    dibujarBoton(3,0,2*CUADRO);
    dibujarIcono(2,CUADRO,3*CUADRO);
    estadoActual=0;
  }  
  else if(estadoActual==3 && millis()-ultimaLectura > DELAY_OFF)
  { // Paso 1 segundo desde que se pulso el boton de borrar una rutina (lo desactivamos)
    dibujarBoton(5,0,4*CUADRO);
    dibujarIcono(3,CUADRO,5*CUADRO); 
    estadoActual=0;
  }
  else if(estadoActual==4 && millis()-ultimaLectura > DELAY_OFF)
  { // Paso 1 segundo desde que se pulso el boton de cambiar el numero del sensor (lo desactivamos)
    estadoActual=0;
    dibujarBoton(7,2*CUADRO,0);
    dibujarIcono(4,3*CUADRO,CUADRO); 
  }
  else if(estadoActual==5 && millis()-ultimaLectura > DELAY_OFF)
  { // Paso 1 segundo desde que se pulso el boton del boton transmitir rutina (lo desactivamos)
    dibujarBoton(9,2*CUADRO,2*CUADRO);
    estadoActual=0;
    dibujarIcono(5,3*CUADRO,3*CUADRO); 
  }  
  else if(estadoActual==6 && millis()-ultimaLectura > DELAY_OFF)
  { // Paso 1 segundo desde que se pulso el boton del boton reproducir rutina (lo desactivamos)
...
// ----------------------------------------------------------------------------------------------------------------
void botonPulsado(short boton)
{ // Agrega efectos al pulsar alguno de los botones de rutinas (recuadro cuando hacemos doble click)
  short x,y;
  if (boton==1){x=0; y=0;}
  else if (boton==2){x=0; y=2*CUADRO;}
  else if (boton==3){x=0; y=4*CUADRO;}
  else if (boton==4){x=2*CUADRO; y=0;}
  else if (boton==5){x=2*CUADRO; y=2*CUADRO;}
  else if (boton==6){x=2*CUADRO; y=4*CUADRO;}

    for(int i=0;i<4;i++)
    { // Dibuja un recuadro rojo en el boton que se haya presionado dos veces
      tft.drawRect(x+i,y+i,2*CUADRO-2*i,2*CUADRO-2*i, tft.color565(255,0,0)); 
    }
}
...
{ // Diseño de los iconos de la pantalla
  ...
  else if(icono==6 && estadoActual == 6)
  { // Iniciar operacion seleccionada
...
{ // Gestiona el comportamiento del programa en funcion de los botones pulsados y envia los mensajes por puerto serial acorde
  ...
  bool estadoDeTransmision = estadoActual==7 || estadoActual==8 || estadoActual==9 || 
                             estadoActual==10 || estadoActual==11 || estadoActual==12;
  String codigo;
...
{ // actualiza los botones, iconos y texto de la pantalla a medida que se va presionando la misma
  ...
  static long toque; // instante donde se detecta el toque de la pantalla
  ...
  bool btn6 = str_zona=="E3" || str_zona=="F3" || str_zona=="E4" || str_zona=="F4";
   ...
   // Se setea en true si presionamos dos veces alguno de los botones
   bool dobleClick = (btn1 && estadoActual==1)||(btn2 && estadoActual==2)||(btn3 && estadoActual==3)||
  (btn4 && estadoActual==4)||(btn5 && estadoActual==5)||(btn6 && estadoActual==6); 

  
  //******************** Pulsando los botones por primera vez *****************************
  ...
  { // Se presiono el boton (en off) del boton transmitir rutina
    dibujarBoton(10,2*CUADRO,2*CUADRO);
    estadoActual=5;
    dibujarIcono(5,3*CUADRO,3*CUADRO); 
    ultimaLectura = millis();
  }
  else if(btn6 && estadoActual==0 )
  { // Se presiono el boton (en off) del boton reproducir rutina
  ...
  //******************** Pulsando los botones por segunda vez *****************************
  if(dobleClick && (estadoActual==1 || estadoActual==2 || estadoActual==3 || estadoActual==4 || estadoActual==5 || estadoActual==6))
  { // Se presiono el boton (en off) de ejecutar funcion de transmision en tiempo real 
    if((estadoActual==2 || estadoActual==3 || estadoActual==4 || estadoActual==5 || estadoActual==6 ) && rutina == 0)
    ...
    { // si no hay errores operamos normalmente
      ...
      if(estadoActual==7) botonPulsado(1);
      else if(estadoActual==8) botonPulsado(2);      
      else if(estadoActual==9) botonPulsado(3);          
      else if(estadoActual==10) botonPulsado(4);    
      else if(estadoActual==11) botonPulsado(5);          
      else if(estadoActual==12) botonPulsado(6);                
      }
  } 
